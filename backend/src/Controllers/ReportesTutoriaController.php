...
class ReportesTutoriaController
{
    ...
    public function getEstadisticasJornada($currentUser)
    {
        $semestrePeriodoId = $_GET['semestreperiodoid'] ?? null;
        $profesorIdFromRequest = $_GET['profesorid'] ?? null;

        // Si es Admin/Coordinador puede filtrar por profesor. Si es docente, forzamos su ID?
        // Por ahora mantenemos la lógica flexible: si viene profesorId se usa, sino se asume global o del usuario.
        // El frontend enviará los filtros seleccionados.
        $profesorId = $profesorIdFromRequest;

        if (!$semestrePeriodoId) {
            Response::error('Faltan parámetros obligatorios: semestreperiodoid', 400);
        }

        $data = $this->service->getEstadisticasJornada($semestrePeriodoId, $profesorId);
        Response::json($data);
    }

    public function getKpiGestionDocente($currentUser)
    {
        $semestrePeriodoId = $_GET['semestreperiodoid'] ?? null;
        $profesorIdFromRequest = $_GET['profesorid'] ?? null;
        ...
        $data = $this->service->getTasaGestionDocente($semestrePeriodoId, $profesorId);
        Response::json($data);
    }
}
