...
### `GET /api/documentos`
Obtiene los documentos subidos por el docente autenticado.

**Parámetros (Query):**
- `semestreperiodoid` (Opcional): Filtra por periodo/nivel.
...
[
  {
    "id": 1,
    "fecha": "2023-10-27",
    "periodo": "Periodo 2023-2",
    "nivel": "Nivel 1",
    "asignatura": "Matemáticas",
    "formato": "Silabo",
    "archivo": "silabo_mate.pdf",
    "url": "https://...",
    "estado": "ENVIADO"
  }
]
...
**Rol Requerido:** Coordinador (ID: 2)

Obtiene un reporte general de documentos subidos en un periodo y nivel, agrupados por Sección y Asignatura.

**Parámetros (Query):**
- `semestreperiodoid` (Requerido): ID del periodo/nivel a consultar.
    ...
    "asignaturas": [
      {
        ...
        "docente_principal": "Juan Perez",
        "documentos": [
          {
            "documentoid": 123,
            "nombrearchivo": "silabo.pdf",
            "url": "https://...",
            "fecha": "2023-10-27",
            "tipo": "Silabo",
            "docente": "Juan Perez",
...
### 8. KPI: Tasa de Gestión Docente
- **Ruta**: `GET /api/reportes-tutoria/kpi-gestion-docente`
- **Parámetros Query**:
  - `semestreperiodoid`: (Obligatorio)
  - `profesorid`: (Opcional)
- **Descripción**: Calcula el porcentaje de cumplimiento de tutorías (Realizadas vs Requeridas). Muestra la tendencia respecto al periodo anterior (si existe).
- **Respuesta**:
  ```json
  {
    "value": 85.5,
...
### 9. KPI: Asistencia Efectiva
- **Ruta**: `GET /api/reportes-tutoria/kpi-asistencia-efectiva`
- **Parámetros Query**: `semestreperiodoid`, `profesorid` (opcional).
- **Descripción**: Mide el porcentaje de asistencia real sobre los eventos finalizados (Realizada vs Inasistencia). Ignora pendientes o incompletas.
- **Respuesta**:
  ```json
  {
    "value": 92.0,
...
### 10. KPI: Impacto Académico Global
- **Ruta**: `GET /api/reportes-tutoria/kpi-impacto-academico`
- **Parámetros Query**: `semestreperiodoid`, `profesorid` (opcional).
- **Descripción**: Mide el promedio de mejora académica (NotaP2 - NotaP1) en estudiantes tutorados.
- **Respuesta**:
  ```json
  {
    "value": 1.5,
    ...
    "meta": {
      "estudiantes_evaluados": 12
    }
  }
  ```

### 11. Estadísticas por Jornada
- **Ruta**: `GET /api/reportes-tutoria/estadisticas-jornada`
- **Parámetros Query**:
  - `semestreperiodoid`: (Obligatorio) ID del periodo.
  - `profesorid`: (Opcional) Filtrar por docente.
- **Descripción**: Obtiene datos agrupados para el gráfico de "Participación y Asistencia por Jornada". Devuelve tres series (Realizada, Incompleta, Inasistencia) distribuidas en Mañana, Tarde y Noche.
- **Respuesta**:
  ```json
  {
